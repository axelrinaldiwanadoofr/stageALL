...
class CmsObject extends CmsNode
	{
	protected $libelle ;  // Libellé
	protected $childs ;   // Noeud fils
	...
	function createContener( &$appctx )
		{
		if( !self::$t_cms_node )
			{
			self::$t_cms_node = new SqlTableContener( $appctx->db, "cms_node", "url,page,id" ) ;
			}
		return self::$t_cms_node ;
		}
	// Ajoute un noeud fils
	...
	function setParentChildLink( &$cms )
		...
		if( $this->idparent != "-1" )
			{
			$this->parent = $cms->getNodeById( $this->idparent ) ;
